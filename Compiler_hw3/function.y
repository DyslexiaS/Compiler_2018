...
;
function_call
    :   F_NAME LB argument_send RB { $$.type = I_CONST; fprintf(fp, "\tjsr FUNC_%s\n", $1); }
;
argument_send
...
for_stat
    ...
    | for_ assign_stat ';' { print_for_start();}
      boolean_expr { print_jump($5, "FOR_EXIT", FOR_IDX );} ';' incdec_stat 
      block { print_jump(-1,"FOR_START", FOR_IDX); print_label("FOR_EXIT", FOR_IDX);}
;
for_
    : FOR { print_for_start();}
;
